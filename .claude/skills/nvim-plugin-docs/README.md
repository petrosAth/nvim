...
1. **Context7** — `resolve-library-id` + `query-docs`. Fastest; use known-good
   IDs for `/neovim/neovim`, `/folke/lazy.nvim`, `/neovim/nvim-lspconfig`,
   `/rebelot/heirline.nvim` without resolving.
2. **Local lazy clone** — `~/.local/share/nvim/lazy/<repo>/`. The exact
   installed version (matches `lazy-lock.json`). Reads `doc/*.txt` → `README.md`
