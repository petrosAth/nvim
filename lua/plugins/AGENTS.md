...
siblings are helper modules: `lsp/`, `heirline/`, `telescope/`, `tabby/`,
...
`<repo>` name, `require` **module**, its **command** names (`Telescope`,
`Neotree`, `UndotreeToggle`, …), its **highlight-group prefix** (`GitSigns`,
`Telescope`, …), and its **filetype** — then sweep:
...
| `lua/styling.lua`                                                                                         | plugin icons/separators (`icons.git.signs.*`, `icons.git.signs.satellite.*`, `icons.lazy.*`, `icons.telescope`, `icons.undoTree`, `icons.diffview`, `icons.alphaCursor`, `separators.hlslens.*`)                                                                                                                      | grep name / icon key                              |
...
| `lua/ui/utilities.lua`                                                                                    | buffer/window labels keyed by plugin filetype/buftype (`aerial`, `Glance`, `trouble`, `undotree`, `Outline`, `TelescopePrompt`, `neo-tree …`)                                                                                                                                                                         | grep the filetype string                          |
...
| other `lua/plugins/**`                                                                                    | **dependency edges** (`dependencies = {}`) and cross-file `require("<module>")` integrations (web-devicons→heirline/tabby/neo-tree; treesitter→rainbow/textobjects/ts-context-commentstring; snacks→neo-tree/lensline/illuminate; null-ls→lsp+heirline; telescope→dressing/trouble; comment→ts-context-commentstring) | the two greps above                               |
